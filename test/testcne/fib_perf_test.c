...
struct route_rule_count {
    uint32_t a[CNE_FIB_MAX_DEPTH];
    uint32_t b[CNE_FIB_MAX_DEPTH];
    uint32_t c[CNE_FIB_MAX_DEPTH];
};
...
static void
generate_random_rule_prefix(uint32_t ip_class, uint8_t depth)
{
...
#define IP_HEAD_BIT_NUM_C 3
    ...
    for (k = 0; k < rule_num; k++) {
        ptr_rule->ip    = (start << (CNE_FIB_MAX_DEPTH - depth)) | ip_head_mask;
...
static void
insert_rule_in_random_pos(uint32_t ip, uint8_t depth)
{
    uint32_t pos;
    int try_count = 0;
...
static void
generate_large_route_rule_table(void)
{
    uint32_t ip_class;
    uint8_t depth;
    ...
    for (ip_class = IP_CLASS_A; ip_class <= IP_CLASS_C; ip_class++) {
        for (depth = 1; depth <= CNE_FIB_MAX_DEPTH; depth++)
     ...
     * with depth = 8.
     */
    insert_rule_in_random_pos(CNE_IPV4(0, 0, 0, 0), 8);
    insert_rule_in_random_pos(CNE_IPV4(10, 2, 23, 147), 32);
    insert_rule_in_random_pos(CNE_IPV4(192, 168, 100, 10), 24);
    insert_rule_in_random_pos(CNE_IPV4(192, 168, 25, 100), 24);
    insert_rule_in_random_pos(CNE_IPV4(192, 168, 129, 124), 32);
}
...
test_fib_perf(void)
{
    ...
    for (i = 0; i < NUM_ROUTE_ENTRIES; i++) {
        if (cne_fib_add(fib, large_route_table[i].ip, large_route_table[i].depth, next_hop_add) ==
            0)
            status++;
    }
    /* End Timer. */
    ...
    cne_printf("The fib_perf result is as follows:\n");
    ...
    for (i = 0; i < ITERATIONS; i++) {
        ...
        for (j = 0; j < BATCH_SIZE; j += BULK_SIZE) {
            uint32_t k;
            cne_fib_lookup_bulk(fib, &ip_batch[j], next_hops, BULK_SIZE);
    ...
    begin  = cne_rdtsc();
    for (i = 0; i < NUM_ROUTE_ENTRIES; i++) {
        /* cne_lpm_delete(lpm, ip, depth) */
        status += cne_fib_delete(fib, large_route_table[i].ip, large_route_table[i].depth);
    }
...
int
fib_perf_main(int argc, char **argv)
{
    tst_info_t *tst;
    int opt, flags = 0;
    char **argvopt;
