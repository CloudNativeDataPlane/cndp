...
#include <cne_meter.h>         // for cne_color, cne_meter_trtcm_profile_config

#include "meter_test.h"
#include "cne_system.h"        // for cne_get_timer_hz

#define cne_get_tsc_cycles cne_rdtsc
#define cne_get_tsc_hz     cne_get_timer_hz
...
static struct cne_meter_srtcm_params sparams = {
    .cir = TM_TEST_SRTCM_CIR_DF,
    .cbs = TM_TEST_SRTCM_CBS_DF,
    .ebs = TM_TEST_SRTCM_EBS_DF,
};

static struct cne_meter_trtcm_params tparams = {
...
static struct cne_meter_trtcm_rfc4115_params rfc4115params = {
    .cir = TM_TEST_TRTCM_CIR_DF,
...
/**
 * functional test for cne_meter_srtcm_config
 */
static inline int
tm_test_srtcm_config(void)
{
#define SRTCM_CFG_MSG "srtcm_config"
    struct cne_meter_srtcm_profile sp;
    struct cne_meter_srtcm_params sparams1;

    /* invalid parameter test */
    if (cne_meter_srtcm_profile_config(NULL, NULL) == 0)
        melog(SRTCM_CFG_MSG);
    if (cne_meter_srtcm_profile_config(&sp, NULL) == 0)
        melog(SRTCM_CFG_MSG);
    if (cne_meter_srtcm_profile_config(NULL, &sparams) == 0)
        melog(SRTCM_CFG_MSG);
    ...
    sparams1.cbs = 0;
    sparams1.ebs = 0;
    if (cne_meter_srtcm_profile_config(&sp, &sparams1) == 0)
        melog(SRTCM_CFG_MSG);
    ...
    if (cne_meter_srtcm_profile_config(&sp, &sparams) != 0)
        melog(SRTCM_CFG_MSG);
...
/**
 * functional test for cne_meter_trtcm_config
 */
static inline int
tm_test_trtcm_config(void)
{
    struct cne_meter_trtcm_profile tp;
    struct cne_meter_trtcm_params tparams1;
#define TRTCM_CFG_MSG "trtcm_config"

    /* invalid parameter test */
    if (cne_meter_trtcm_profile_config(NULL, NULL) == 0)
        melog(TRTCM_CFG_MSG);
    if (cne_meter_trtcm_profile_config(&tp, NULL) == 0)
        melog(TRTCM_CFG_MSG);
    if (cne_meter_trtcm_profile_config(NULL, &tparams) == 0)
        melog(TRTCM_CFG_MSG);

    /* cir, cbs, pir and pbs never be zero */
    tparams1     = tparams;
    tparams1.cir = 0;
    if (cne_meter_trtcm_profile_config(&tp, &tparams1) == 0)
        melog(TRTCM_CFG_MSG);
    ...
    /* usual parameter, should be successful */
    if (cne_meter_trtcm_profile_config(&tp, &tparams) != 0)
        melog(TRTCM_CFG_MSG);
...
/**
 * functional test for cne_meter_trtcm_rfc4115_config
 */
static inline int
tm_test_trtcm_rfc4115_config(void)
{
    struct cne_meter_trtcm_rfc4115_profile tp;
    struct cne_meter_trtcm_rfc4115_params rfc4115params1;
#define TRTCM_RFC4115_CFG_MSG "trtcm_rfc4115_config"

    /* invalid parameter test */
    if (cne_meter_trtcm_rfc4115_profile_config(NULL, NULL) == 0)
        melog(TRTCM_RFC4115_CFG_MSG);
    if (cne_meter_trtcm_rfc4115_profile_config(&tp, NULL) == 0)
        melog(TRTCM_RFC4115_CFG_MSG);
    if (cne_meter_trtcm_rfc4115_profile_config(NULL, &rfc4115params) == 0)
        melog(TRTCM_RFC4115_CFG_MSG);
    ...
    if (cne_meter_trtcm_rfc4115_profile_config(&tp, &rfc4115params1) == 0)
        melog(TRTCM_RFC4115_CFG_MSG);
    ...
    /* usual parameter, should be successful */
    if (cne_meter_trtcm_rfc4115_profile_config(&tp, &rfc4115params) != 0)
        melog(TRTCM_RFC4115_CFG_MSG);
...
/**
 * functional test for cne_meter_srtcm_color_blind_check
 */
static inline int
...
#define SRTCM_BLIND_CHECK_MSG "srtcm_blind_check"
    struct cne_meter_srtcm_profile sp;
    struct cne_meter_srtcm sm;
    uint64_t time;
    uint64_t hz = cne_get_tsc_hz();

    /* Test green */
    if (cne_meter_srtcm_profile_config(&sp, &sparams) != 0)
        melog(SRTCM_BLIND_CHECK_MSG);
    if (cne_meter_srtcm_config(&sm, &sp) != 0)
        melog(SRTCM_BLIND_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_srtcm_color_blind_check(&sm, &sp, time, TM_TEST_SRTCM_CBS_DF - 1) !=
        CNE_COLOR_GREEN)
    ...
    if (cne_meter_srtcm_config(&sm, &sp) != 0)
        melog(SRTCM_BLIND_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_srtcm_color_blind_check(&sm, &sp, time, (uint32_t)sp.ebs - 1) != CNE_COLOR_YELLOW)
        melog(SRTCM_BLIND_CHECK_MSG " YELLOW");
    ...
    if (cne_meter_srtcm_config(&sm, &sp) != 0)
        melog(SRTCM_BLIND_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_srtcm_color_blind_check(&sm, &sp, time, TM_TEST_SRTCM_EBS_DF + 1) !=
        CNE_COLOR_RED)
...
/**
 * functional test for cne_meter_trtcm_color_blind_check
 */
static inline int
...
#define TRTCM_BLIND_CHECK_MSG "trtcm_blind_check"
    ...
    uint64_t time;
    struct cne_meter_trtcm_profile tp;
    struct cne_meter_trtcm tm;
    uint64_t hz = cne_get_tsc_hz();

    /* Test green */
    if (cne_meter_trtcm_profile_config(&tp, &tparams) != 0)
        melog(TRTCM_BLIND_CHECK_MSG);
    if (cne_meter_trtcm_config(&tm, &tp) != 0)
        melog(TRTCM_BLIND_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_trtcm_color_blind_check(&tm, &tp, time, TM_TEST_TRTCM_CBS_DF - 1) !=
        CNE_COLOR_GREEN)
    ...
    if (cne_meter_trtcm_config(&tm, &tp) != 0)
        melog(TRTCM_BLIND_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_trtcm_color_blind_check(&tm, &tp, time, TM_TEST_TRTCM_PBS_DF - 1) !=
        CNE_COLOR_YELLOW)
        melog(TRTCM_BLIND_CHECK_MSG " YELLOW");
...
/**
 * functional test for cne_meter_trtcm_rfc4115_color_blind_check
 */
static inline int
...
#define TRTCM_RFC4115_BLIND_CHECK_MSG "trtcm_rfc4115_blind_check"
    ...
    uint64_t time;
    struct cne_meter_trtcm_rfc4115_profile tp;
    struct cne_meter_trtcm_rfc4115 tm;
    uint64_t hz = cne_get_tsc_hz();

    /* Test green */
    if (cne_meter_trtcm_rfc4115_profile_config(&tp, &rfc4115params) != 0)
        melog(TRTCM_RFC4115_BLIND_CHECK_MSG);
    if (cne_meter_trtcm_rfc4115_config(&tm, &tp) != 0)
        melog(TRTCM_RFC4115_BLIND_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_trtcm_rfc4115_color_blind_check(&tm, &tp, time, TM_TEST_TRTCM_CBS_DF - 1) !=
        CNE_COLOR_GREEN)
    ...
    if (cne_meter_trtcm_rfc4115_config(&tm, &tp) != 0)
        melog(TRTCM_RFC4115_BLIND_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_trtcm_rfc4115_color_blind_check(&tm, &tp, time, TM_TEST_TRTCM_EBS_DF - 1) !=
        CNE_COLOR_YELLOW)
        melog(TRTCM_RFC4115_BLIND_CHECK_MSG " YELLOW");
...
static inline int
tm_test_srtcm_aware_check(enum cne_color in[4], enum cne_color out[4])
{
#define SRTCM_AWARE_CHECK_MSG "srtcm_aware_check"
    ...
    if (cne_meter_srtcm_config(&sm, &sp) != 0)
        melog(SRTCM_AWARE_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_srtcm_color_aware_check(&sm, &sp, time, TM_TEST_SRTCM_CBS_DF - 1, in[0]) !=
        out[0])
    ...
    if (cne_meter_srtcm_config(&sm, &sp) != 0)
        melog(SRTCM_AWARE_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_srtcm_color_aware_check(&sm, &sp, time, TM_TEST_SRTCM_CBS_DF + 1, in[1]) !=
        out[1])
    ...
    if (cne_meter_srtcm_config(&sm, &sp) != 0)
        melog(SRTCM_AWARE_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_srtcm_color_aware_check(&sm, &sp, time, TM_TEST_SRTCM_EBS_DF - 1, in[2]) !=
        out[2])
    ...
    if (cne_meter_srtcm_config(&sm, &sp) != 0)
        melog(SRTCM_AWARE_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_srtcm_color_aware_check(&sm, &sp, time, TM_TEST_SRTCM_EBS_DF + 1, in[3]) !=
        out[3])
...
/**
 * functional test for cne_meter_srtcm_color_aware_check
 */
static inline int
tm_test_srtcm_color_aware_check(void)
{
    enum cne_color in[4], out[4];
     ...
     * yellow & red = red
     */
    in[0] = in[1] = in[2] = in[3] = CNE_COLOR_YELLOW;
    out[0]                        = CNE_COLOR_YELLOW;
    out[1]                        = CNE_COLOR_YELLOW;
    out[2]                        = CNE_COLOR_YELLOW;
    out[3]                        = CNE_COLOR_RED;
    if (tm_test_srtcm_aware_check(in, out) != 0)
     ...
     * red & yellow = red
     */
    in[0] = in[1] = in[2] = in[3] = CNE_COLOR_RED;
    out[0]                        = CNE_COLOR_RED;
    out[1]                        = CNE_COLOR_RED;
    out[2]                        = CNE_COLOR_RED;
    out[3]                        = CNE_COLOR_RED;
 ...
 */
static inline int
tm_test_trtcm_aware_check(enum cne_color in[4], enum cne_color out[4])
{
#define TRTCM_AWARE_CHECK_MSG "trtcm_aware_check"
    ...
    if (cne_meter_trtcm_config(&tm, &tp) != 0)
        melog(TRTCM_AWARE_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_trtcm_color_aware_check(&tm, &tp, time, TM_TEST_TRTCM_CBS_DF - 1, in[0]) !=
        out[0])
    ...
    if (cne_meter_trtcm_config(&tm, &tp) != 0)
        melog(TRTCM_AWARE_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_trtcm_color_aware_check(&tm, &tp, time, TM_TEST_TRTCM_CBS_DF + 1, in[1]) !=
        out[1])
    ...
    if (cne_meter_trtcm_config(&tm, &tp) != 0)
        melog(TRTCM_AWARE_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_trtcm_color_aware_check(&tm, &tp, time, TM_TEST_TRTCM_PBS_DF - 1, in[2]) !=
        out[2])
    ...
    if (cne_meter_trtcm_config(&tm, &tp) != 0)
        melog(TRTCM_AWARE_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_trtcm_color_aware_check(&tm, &tp, time, TM_TEST_TRTCM_PBS_DF + 1, in[3]) !=
        out[3])
...
/**
 * functional test for cne_meter_trtcm_color_aware_check
 */
 ...
 */
static inline int
tm_test_trtcm_rfc4115_aware_check(enum cne_color in[4], enum cne_color out[4])
{
#define TRTCM_RFC4115_AWARE_CHECK_MSG "trtcm_rfc4115_aware_check"
    ...
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_trtcm_rfc4115_color_aware_check(&tm, &tp, time, TM_TEST_TRTCM_CBS_DF - 1,
                                                  in[0]) != out[0])
    ...
    if (cne_meter_trtcm_rfc4115_config(&tm, &tp) != 0)
        melog(TRTCM_RFC4115_AWARE_CHECK_MSG);
    time = cne_get_tsc_cycles() + hz;
    if (cne_meter_trtcm_rfc4115_color_aware_check(&tm, &tp, time, TM_TEST_TRTCM_EBS_DF - 1,
                                                  in[2]) != out[2])
...
/**
 * functional test for cne_meter_trtcm_rfc4115_color_aware_check
 */
static inline int
