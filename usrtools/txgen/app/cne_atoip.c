...
#include <string.h>            // for memcpy, strchr, memset, strrchr
...
inet_ipton6(const char *src, unsigned char *dst)
{
    ...
    while ((ch = *src++) != '\0') {
        ...
        if (ch == '.' && ((tp + _INADDRSZ) <= endp) && inet_ipton4(curtok, tp) > 0) {
            tp += _INADDRSZ;
            saw_xdigit = 0;
            dbloct_count += 2;
...
_atoip(const char *buf, int flags, void *res, unsigned ressize)
{
    ...
    if (flags & _IPADDR_NETWORK) {
        ...
        *prefix = '\0';
        ...
        if (errno || (*prefix_end != '\0') || prefixlen < 0 || prefixlen > _PREFIXMAX)
            return -1;
    ...
    /* convert the IP addr */
    if ((flags & _IPADDR_V4) && inet_ipton(AF_INET, ip_str, &ipaddr.ipv4) == 1 &&
