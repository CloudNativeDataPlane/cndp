...
static int
udp_close(int cd __cne_unused)
{
    CNE_NOTICE("Close channel %d\n", cd);
    return 0;
}
...
tcp_accept(int cd)
{
    ...
    if (ncd < 0)
        CNE_WARN("Accept returned an error from %d descriptor\n", cd);
    CNE_NOTICE("Accept new chnl %d\n", ncd);
...
tcp_recv(int cd)
{
    ...
    case LOOPBACK_TEST:
        ...
        CNE_NOTICE("Loopback Received %d\n", nb_mbufs);
        if (chnl_send(cd, mbufs, nb_mbufs) < 0) {
            pktmbuf_free_bulk(mbufs, nb_mbufs);
            CNE_ERR("Unable to send packets\n");
        }
...
static int
proto_callback(chnl_type_t ctype, int cd)
{
