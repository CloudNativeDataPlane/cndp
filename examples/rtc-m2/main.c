...
static int
_rtc_m2_test(jcfg_lport_t *lport, struct fwd_info *fwd)
{
...
thread_func(void *arg)
{
    ...
    cne_printf("   [green]Thread ID [orange]%d [green]on lcore [orange]%d[]\n", thd->tid,
               cne_lcore_id());
...
main(int argc, char **argv)
{
    ...
    cne_printf("\n[yellow]*** [cyan:-:italic]RTC-M2 Application[], "
    ...
    /* don't start any threads before we initialize */
    if (pthread_barrier_wait(&fwd->barrier) > 0)
...
err:
    ...
    cne_printf_pos(99, 1,
                   "\n*** [cyan]RTC-M2 Application[], [green]API[]: [magenta]%s [green]PID[]: "
                   "[magenta]%d[] failed\n",
