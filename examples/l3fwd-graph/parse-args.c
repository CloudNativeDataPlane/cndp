...
process_callback(jcfg_info_t *j __cne_unused, void *_obj, void *arg, int idx)
{
    ...
    case JCFG_THREAD_TYPE:
        if (!strcasecmp("main", obj.thd->thread_type)) { /* Main thread */
            ...
            if (thread_create(obj.thd->name, print_stats, obj.thd) < 0)
            ...
            CNE_WARN("*** Unknown thread type '%s'\n", obj.thd->thread_type);
        break;
...
parse_args(int argc, char **argv)
{
    ...
    CNE_INFO("Setup barrier for %d threads\n", jcfg_num_threads(fwd->jinfo));
    ...
    if (!fwd->opts.no_metrics && enable_metrics())
