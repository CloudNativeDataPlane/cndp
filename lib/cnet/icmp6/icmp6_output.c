...
icmp6_output_header(pktmbuf_t *m)
{
    ...
    return ICMP6_OUTPUT_NEXT_IP6_OUTPUT;
}
...
icmp6_output_node_do(struct cne_graph *graph, struct cne_node *node, void **objs, uint16_t nb_objs)
{
    ...
    while (n_left_from >= 4) {
        ...
        next0 = icmp6_output_header(mbuf0);
        next1 = icmp6_output_header(mbuf1);
        next2 = icmp6_output_header(mbuf2);
        next3 = icmp6_output_header(mbuf3);
