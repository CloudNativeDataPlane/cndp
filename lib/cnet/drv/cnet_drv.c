...
#include <cne_vec.h>           // for vec_len, vec_set_len, vec_ptr_at_index, vec_max...
...
drv_setup(struct cnet *cnet, int lport)
{
    ...
    vec_add_at_index(cnet->drvs, drv, lport);
...
cnet_drv_destroy(struct cnet *cnet)
{
    if (!cnet || !cnet->drvs)
        return -1;
    for (int i = 0; i < vec_max_len(cnet->drvs); i++) {
        struct drv_entry *drv;

        drv = vec_ptr_at_index(cnet->drvs, i);
        if (drv == NULL)
            continue;
        free(drv);
    }
