...
tcp_send_header(pktmbuf_t *m, uint16_t nxt)
{
    ...
    nxt = TCP_SEND_NEXT_IP4_OUTPUT;

    return nxt;
}

static uint16_t
tcp_send_node_inline(struct cne_graph *graph, struct cne_node *node, void **objs, uint16_t nb_objs)
{
    ...
    next_index = TCP_SEND_NEXT_IP4_OUTPUT;
    ...
    while (n_left_from >= 4) {
        ...
        next0 = tcp_send_header(mbuf0, TCP_SEND_NEXT_PKT_DROP);
        next1 = tcp_send_header(mbuf1, TCP_SEND_NEXT_PKT_DROP);
        next2 = tcp_send_header(mbuf2, TCP_SEND_NEXT_PKT_DROP);
        next3 = tcp_send_header(mbuf3, TCP_SEND_NEXT_PKT_DROP);
...
static uint16_t
tcp_send_node_process(struct cne_graph *graph, struct cne_node *node, void **objs, uint16_t nb_objs)
{
    tcp_send_node_ctx_t *ctx = (tcp_send_node_ctx_t *)node->ctx;
    uint64_t tsc             = cne_rdtsc();

    CNE_SET_USED(nb_objs);

    /* Attempt to process the TCP protocol timers every 10ms */
    if (tsc >= ctx->interval) {
        stk_t *stk         = this_stk;
        uint64_t timo_10ms = (cne_get_timer_hz() / (1000 / MS_PER_TICK));

        ctx->interval = tsc + timo_10ms;

        stk->ticks++; /* Each tick is 10ms */

        /* Process the TCP timers */
        __tcp_process_timers();
    }

    return (node->idx) ? tcp_send_node_inline(graph, node, objs, node->idx) : 0;
}

static int
tcp_send_node_init(const struct cne_graph *graph __cne_unused, struct cne_node *node)
{
    tcp_send_node_ctx_t *ctx = (tcp_send_node_ctx_t *)node->ctx;

    /* Next timeout interval is cycles */
    ctx->interval = cne_rdtsc() + (cne_get_timer_hz() / MS_PER_TICK);

    return 0;
}

static struct cne_node_register tcp_send_node_base = {
    .process = tcp_send_node_process,
    .flags   = CNE_NODE_INPUT_F,
    .name    = "tcp_send",

    .init = tcp_send_node_init,

    .nb_edges = TCP_SEND_NEXT_MAX,
    .next_nodes =
        {
            [TCP_SEND_NEXT_PKT_DROP]   = "pkt_drop",
            [TCP_SEND_NEXT_IP4_OUTPUT] = "ip4_output",
        },
};

CNE_NODE_REGISTER(tcp_send_node_base);
