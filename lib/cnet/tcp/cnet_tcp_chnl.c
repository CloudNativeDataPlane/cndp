...
static int
tcp_chnl_accept(int cd, struct in_caddr *addr, int *addrlen)
{
...
static int
tcp_chnl_bind(int cd, struct in_caddr *addr, int32_t len)
{
    struct chnl *ch = ch_get(cd);
    struct tcb_entry *tcb;
    ...
    if (chnl_bind_common(cd, addr, len, &this_stk->tcp->tcp_hd) == -1)
...
static int
tcp_connect(int cd, struct in_caddr *to __cne_unused, int slen __cne_unused)
{
    struct chnl *ch = ch_get(cd);
    struct tcb_entry *tcb;
...
static int
tcp_chnl_connect(int cd, struct in_caddr *to, int slen)
{
    return tcp_connect(cd, to, slen);
}

static int
tcp_chnl_listen(int cd, int32_t backlog)
{
    struct chnl *ch = ch_get(cd);
    struct tcb_entry *tcb;
...
static int
tcp_chnl_recv(int cd, pktmbuf_t **mbufs, int nb_mbufs)
{
    struct chnl *ch = ch_get(cd);
    uint32_t sz     = 0;
    int tlen, n = 0;
 ...
 */
static int
tcp_chnl_send(int cd, pktmbuf_t **mbufs, uint16_t nb_mbufs)
{
    struct chnl *ch = ch_get(cd);
 ...
 */
static int
tcp_chnl_shutdown(int cd, int32_t how)
{
    struct chnl *ch = ch_get(cd);
...
static int
tcp_chnl_opt_set(int cd, int level, int optname, const void *optval, uint32_t optlen)
{
    uint32_t val;
    struct chnl *ch = ch_get(cd);
...
static int
tcp_chnl_opt_get(int cd, int level, int optname, void *optval, uint32_t *optlen)
{
    struct chnl *ch      = ch_get(cd);
    uint64_t opt[8]      = {0};
    void *resP           = (void *)opt;
