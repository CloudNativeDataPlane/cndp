 ...
 * @file
 *
 * Functions for SSE/AVX/AVX2/AVX512 implementation of memcpy() suitable
 * for copying large packets.
 */
 ...
 *
 * @param dst
 *   The memory address to copy the data too
 * @param src
 *   The memory address to copy the data from
 * @param n
 ...
 * Requirements:
 * - Store is aligned
 * - Load offset is \p offset, which must be immediate value within [1, 15]
 * - For \p src, make sure \p offset bit backwards & \<16 - offset\> bit forwards are available
 * for loading
 * - \p dst, \p src, \p len must be variables
 ...
 * Requirements:
 * - Store is aligned
 * - Load offset is \p offset, which must be within [1, 15]
 * - For \p src, make sure \p offset bit backwards & <16 - offset> bit forwards are available for loading
 * - \p dst, \p src, \p len must be variables
