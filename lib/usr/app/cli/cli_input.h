...
#include "cli.h"        // for cli, this_cli, cli_clr_flag, cli_tst_flag
#include "cli_common.h"
#include "cli_gapbuf.h"        // for gb_data_size, gb_left_data_size, gb_point_of...
#include "cne_log.h"
#include "cne_common.h"        // for CNDP_API
#include "cne_stdio.h"         // for cne_printf
#include "cne_tty.h"           // for tty_write, tty_num_columns, tty_read
#include "vt100_out.h"         // for vt_bol, vt_cnright, vt_cpos, vt100_clear_line
...
{
    tty_write(vt100_restore_cursor, -1);
}
...
cli_display_left(void)
{
    if (gb_left_data_size(this_cli->gb))
        tty_write(gb_start_of_buf(this_cli->gb), gb_left_data_size(this_cli->gb));
}
...
cli_display_right(void)
{
    if (gb_right_data_size(this_cli->gb))
        tty_write(gb_end_of_gap(this_cli->gb), gb_right_data_size(this_cli->gb));
}
...
{
    tty_write(vt100_clear_screen, -1);
}
...
{
    if (lineno > 0)
        cne_printf(vt100_pos_cursor, lineno, 0);
    else
        tty_write("\r", 1);

    tty_write(vt100_clear_line, -1);
}
...
cli_display_line(void)
{
    ...
    int window = (tty_num_columns() - this_cli->plen) - 1;
    int wstart, wend;
    ...
    vt_bol();
    vt_cnright(this_cli->plen);

    (void)wend;
    tty_write(&buf[wstart], wend - wstart);

    cli_clear_to_eol();

    vt_bol();
    vt_cnright(this_cli->plen + point);
}
