...
Data Plane Development Kit (CNDP). The CLI is a workalike replacement for
cmdline library in CNDP and has a simpler programming interface and programming
model.
...
controlling a CNDP application. The directory style design is nothing new, but
 ...
 * Added support for color and cursor movement APIs similar to TXGen if needed by the developer.
 ...
 * No weird structures in the code and reduces the line count for testcne from 12K to 4.5K lines. I convert testcne to have both CMDLINE and CLI with a command line option.
...
use of the command line interface in the CNDP. This application is a
readline-like interface that can be used to control a CNDP application.
...
.. code-block:: console
       ...
       export CNE_SDK=/path/to/cne_sdk
       cd ${CNE_SDK}/examples/cli

#.  Set the target (a default target is used if not specified). For example:

.. code-block:: console

       export CNE_TARGET=x86_64-native-linux-gcc
	   or
       export CNE_TARGET=x86_64-native-linuxapp-gcc

   Refer to the *CNDP Getting Started Guide* for possible CNE_TARGET values.
...
.. note::
   ...
   as it does not startup CNDP by calling cne_eal_init() routine. Which means
   it also does not use CNDP features except for a few routines not requiring
   CNE initialization.

Refer to the *CNDP Getting Started Guide* for general information on running applications
and the Cloud Native Environment (CNE) options.
...
CNE Initialization and cmdline Start
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. code-block:: c
   ...
   *   0 on success or -1
   */
   int cli_create(struct cli_cfg *cfg);
	...
	show_cmd(int argc, char **argv)
	{
		...
		switch(m->index) {
			...
			case 20:
				...
				cne_printf("   Show Portlist: %08x, MAC: "
			...
			case 30:
				...
				cne_printf("   Show Portlist: %08x vlan %d MAC: "
						"%02x:%02x:%02x:%02x:%02x:%02x\n",
						   portlist,
						   atoi(argv[3]),
						   mac.ether_addr_octet[0],
...
The following is from TXGen source code to add more help to the global
help for the system.
...
aliases as needed, which is different from the cmdline interface in CNDP today.
...
seen in the lib/libcne_cli/cli_cmds.c file.
