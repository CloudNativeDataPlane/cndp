...
#include "cli.h"               // for cli_node, get_cwd, is_directory, cli_root_node
#include "cli_search.h"        // for args_t, arg_u, cli_scan_t, cli_dir_item_count
...
cli_search_dir(struct cli_node *dir, const char *name, uint32_t type)
{
    ...
    TAILQ_FOREACH (node, &dir->items, next) {
        if (!strcmp(node->name, name) && (node->type & type))
            return node;
    }
...
cli_find_node(const char *path, struct cli_node **ret)
{
    ...
    n = cne_strtok(my_path, "/", argv, CLI_MAX_ARGVS);
