...
/**
 * @file
 * CNE IPv6 Longest Prefix Match (LPM)
 */
...
struct cne_trie_tbl {
    ...
    enum cne_fib_trie_nh_sz nh_sz; /**< Size of nexthop entry */
...
static inline void *
get_tbl24_p(struct cne_trie_tbl *dp, const uint8_t *ip, uint8_t nh_sz)
{
    uint32_t tbl24_idx;
...
{
    return ((tbl[get_tbl_pos(idx, nh_sz)] >> (get_psd_idx(idx, nh_sz) * bits_in_nh(nh_sz))) &
            lookup_msk(nh_sz));
}
    ...
    {                                                                                           \
        ...
        for (i = 0; i < n; i++) {                                                               \
            ...
            while (is_entry_extended(tmp)) {                                                    \
                tmp = ((type *)dp->tbl8)[ips[i][j++] + ((tmp >> 1) * TRIE_TBL8_GRP_NUM_ENT)];   \
            }                                                                                   \
            next_hops[i] = tmp >> 1;                                                            \
        }                                                                                       \
    }
