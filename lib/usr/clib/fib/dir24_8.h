...
#include <stdint.h>                       // for uint32_t, uint64_t, uint8_t, uint...

#include "cne_common.h"        // for CNE_MIN, __cne_cache_aligned
#include "cne_fib.h"           // for cne_fib_conf (ptr only), cne_fib_...
...
struct dir24_8_tbl {
    ...
    enum cne_fib_dir24_8_nh_sz nh_sz; /**< Size of nexthop entry */
    uint64_t def_nh;                  /**< Default next hop */
    uint64_t *tbl8;                   /**< tbl8 table. */
    uint64_t *tbl8_idxes;             /**< bitmap containing free tbl8 idxes*/
    /* tbl24 table. */
    __extension__ uint64_t tbl24[0] __cne_cache_aligned;
};
...
#define LOOKUP_FUNC(suffix, type, bulk_prefetch, nh_sz)                                            \
    ...
    {                                                                                              \
        ...
        uint64_t tmp;                                                                              \
        uint32_t i;                                                                                \
        uint32_t prefetch_offset = CNE_MIN((unsigned int)bulk_prefetch, n);                        \
                                                                                                   \
        for (i = 0; i < prefetch_offset; i++)                                                      \
            cne_prefetch0(get_tbl24_p(dp, ips[i], nh_sz));                                         \
        for (i = 0; i < (n - prefetch_offset); i++) {                                              \
            cne_prefetch0(get_tbl24_p(dp, ips[i + prefetch_offset], nh_sz));                       \
            tmp = ((type *)dp->tbl24)[ips[i] >> 8];                                                \
            if (unlikely(is_entry_extended(tmp)))                                                  \
                tmp =                                                                              \
                    ((type *)dp->tbl8)[(uint8_t)ips[i] + ((tmp >> 1) * DIR24_8_TBL8_GRP_NUM_ENT)]; \
            next_hops[i] = tmp >> 1;                                                               \
        }                                                                                          \
        for (; i < n; i++) {                                                                       \
            tmp = ((type *)dp->tbl24)[ips[i] >> 8];                                                \
...
static inline void
dir24_8_lookup_bulk(struct dir24_8_tbl *dp, const uint32_t *ips, uint64_t *next_hops,
                    const unsigned int n, uint8_t nh_sz)
{
    uint64_t tmp;
    uint32_t i;
    uint32_t prefetch_offset = CNE_MIN(15U, n);
...
static inline void
dir24_8_lookup_bulk_0(void *p, const uint32_t *ips, uint64_t *next_hops, const unsigned int n)
{
    struct dir24_8_tbl *dp = (struct dir24_8_tbl *)p;
...
static inline void
dir24_8_lookup_bulk_1(void *p, const uint32_t *ips, uint64_t *next_hops, const unsigned int n)
{
    struct dir24_8_tbl *dp = (struct dir24_8_tbl *)p;
...
static inline void
dir24_8_lookup_bulk_2(void *p, const uint32_t *ips, uint64_t *next_hops, const unsigned int n)
{
    struct dir24_8_tbl *dp = (struct dir24_8_tbl *)p;
...
static inline void
dir24_8_lookup_bulk_3(void *p, const uint32_t *ips, uint64_t *next_hops, const unsigned int n)
{
    struct dir24_8_tbl *dp = (struct dir24_8_tbl *)p;
...
static inline void
dir24_8_lookup_bulk_uni(void *p, const uint32_t *ips, uint64_t *next_hops, const unsigned int n)
{
    struct dir24_8_tbl *dp = (struct dir24_8_tbl *)p;
    uint64_t tmp;
    uint32_t i;
...
void *dir24_8_create(struct cne_fib_conf *conf);

void dir24_8_free(void *p);

cne_fib_lookup_fn_t dir24_8_get_lookup_fn(void *p, enum cne_fib_lookup_type type);

int dir24_8_modify(struct cne_fib *fib, uint32_t ip, uint8_t depth, uint64_t next_hop, int op);
