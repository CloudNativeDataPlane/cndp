...
#include "cne_common.h"        // for CNE_LEN2MASK
...
#define CNE_ACL_MAX_CATEGORIES 16

#define CNE_ACL_RESULTS_MULTIPLIER (XMM_SIZE / sizeof(uint32_t))

#define CNE_ACL_MAX_LEVELS 64
#define CNE_ACL_MAX_FIELDS 64

union cne_acl_field_types {
    uint8_t u8;
    uint16_t u16;
    uint32_t u32;
    uint64_t u64;
...
enum { CNE_ACL_FIELD_TYPE_MASK = 0, CNE_ACL_FIELD_TYPE_RANGE, CNE_ACL_FIELD_TYPE_BITMASK };
 ...
 * consecutive bytes.
 */
struct cne_acl_field_def {
    uint8_t type;        /**< type - CNE_ACL_FIELD_TYPE_*. */
...
enum {
    CNE_ACL_TYPE_SHIFT   = 29,
    CNE_ACL_MAX_INDEX    = CNE_LEN2MASK(CNE_ACL_TYPE_SHIFT, uint32_t),
    CNE_ACL_MAX_PRIORITY = CNE_ACL_MAX_INDEX,
    CNE_ACL_MIN_PRIORITY = 1,
};
...
#define CNE_ACL_RULE_DEF(name, fld_num)      \
    struct name {                            \
        struct cne_acl_rule_data data;       \
        struct cne_acl_field field[fld_num]; \
    }

CNE_ACL_RULE_DEF(cne_acl_rule, );

#define CNE_ACL_RULE_SZ(fld_num) \
    (sizeof(struct cne_acl_rule) + sizeof(struct cne_acl_field) * (fld_num))
 ...
 */
enum cne_acl_classify_alg {
    CNE_ACL_CLASSIFY_DEFAULT   = 0,
    CNE_ACL_CLASSIFY_SCALAR    = 1, /**< generic implementation. */
    CNE_ACL_CLASSIFY_SSE       = 2, /**< requires SSE4.1 support. */
    CNE_ACL_CLASSIFY_AVX2      = 3, /**< requires AVX2 support. */
    CNE_ACL_CLASSIFY_NEON      = 4, /**< requires NEON support. */
    CNE_ACL_CLASSIFY_ALTIVEC   = 5, /**< requires ALTIVEC support. */
    CNE_ACL_CLASSIFY_AVX512X16 = 6, /**< requires ALTIVEC support. */
    CNE_ACL_CLASSIFY_AVX512X32 = 7, /**< requires ALTIVEC support. */
    CNE_ACL_CLASSIFY_NUM            /* should always be the last one. */
};
 ...
 *   Note that all fields in cne_acl_rule structures are expected
 ...
 * @param cfg
 *   Pointer to struct cne_acl_config - defines build parameters.
 * @return
...
/**
 * Set the acl classify algo for the ACL context and
 * This function is not multi-thread safe.
 *
 * @param ctx
 *   ACL context to reset.
 * @param algo
 *   the type of acl algo
 */
 ...
 * CNE_ACL_RESULTS_MULTIPLIER and can't be bigger than CNE_ACL_MAX_CATEGORIES.
 ...
 */
extern int cne_acl_classify(const struct cne_acl_ctx *ctx, const uint8_t **data, uint32_t *results,
 ...
 *   -EINVAL for incorrect arguments.
 */
extern int cne_acl_classify_alg(const struct cne_acl_ctx *ctx, const uint8_t **data,
 ...
 *   - Zero if operation completed successfully.
 */
extern int cne_acl_set_ctx_classify(struct cne_acl_ctx *ctx, enum cne_acl_classify_alg alg);
