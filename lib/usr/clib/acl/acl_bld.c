...
#include <errno.h>         // for EINVAL, ENOMEM, ERANGE
#include <limits.h>        // for CHAR_BIT
#include <setjmp.h>        // for sigsetjmp
#include <stddef.h>        // for offsetof
#include <stdint.h>        // for uint32_t, uint8_t, int32_t, UINT8_MAX, uint64_t
#include <stdlib.h>        // for free
#include <string.h>        // for NULL, memset, memcpy, size_t, memmove

#include <cne_acl.h>        // for cne_acl_config, cne_acl_field_def, cne_acl_f...

#include "tb_mem.h"            // for tb_alloc, tb_mem_pool, tb_free_pool
#include "acl.h"               // for cne_acl_node, cne_acl_ptr_set, cne_acl_trie
#include "cne_common.h"        // for CNE_DIM, CNE_LEN2MASK, typeof
...
struct cne_acl_build_rule {
    struct cne_acl_build_rule *next;
    struct cne_acl_config *config;
    /**< configuration for each field in the rule. */
    const struct cne_acl_rule *f;
    uint32_t *wildness;
};

/* Context for build phase */
struct acl_build_context {
    const struct cne_acl_ctx *acx;
    struct cne_acl_build_rule *build_rules;
    ...
    struct cne_acl_trie tries[CNE_ACL_MAX_TRIES];
    struct cne_acl_bld_trie bld_tries[CNE_ACL_MAX_TRIES];
    uint32_t data_indexes[CNE_ACL_MAX_TRIES][CNE_ACL_MAX_FIELDS];

    /* memory free lists for nodes and blocks used for node ptrs */
    struct acl_mem_block blocks[MEM_BLOCK_NUM];
    struct cne_acl_node *node_free_list;
};

static int acl_merge_trie(struct acl_build_context *context, struct cne_acl_node *node_a,
                          struct cne_acl_node *node_b, uint32_t level,
                          struct cne_acl_node **node_c);

static void acl_deref_ptr(struct acl_build_context *context, struct cne_acl_node *node, int index);
...
acl_build_alloc(struct acl_build_context *context, size_t n, size_t s)
{
     ...
     */
    for (m = 0; m < CNE_DIM(context->blocks); m++) {
        if (context->blocks[m].block_size == alloc_size && context->blocks[m].mem_ptr != NULL) {
...
acl_build_free(struct acl_build_context *context, size_t s, void *p)
{
    ...
    for (n = 0; n < CNE_DIM(context->blocks); n++) {
 ...
 */
static struct cne_acl_node *
acl_alloc_node(struct acl_build_context *context, int level)
{
    struct cne_acl_node *node = NULL;

    if (context->node_free_list != NULL) {
        node                    = context->node_free_list;
        context->node_free_list = node->next;
        memset(node, 0, sizeof(struct cne_acl_node));
    } else {
        node = acl_build_alloc(context, sizeof(struct cne_acl_node), 1);
    }

    if (node != NULL) {
        node->num_ptrs   = 0;
        node->level      = level;
        node->node_type  = CNE_ACL_NODE_UNDEFINED;
        node->node_index = CNE_ACL_NODE_UNDEFINED;
        context->num_nodes++;
 ...
 */
static void
acl_free_node(struct acl_build_context *context, struct cne_acl_node *node)
{
    uint32_t n;
    ...
    if (node->mrt != NULL) {
        acl_build_free(context, sizeof(struct cne_acl_match_results), node->mrt);
        node->mrt = NULL;
    }

    /* free transitions to other nodes */
    if (node->ptrs != NULL) {
        acl_build_free(context, node->max_ptrs * sizeof(struct cne_acl_ptr_set), node->ptrs);
        node->ptrs = NULL;
    }
 ...
 */
static void
acl_include(struct cne_acl_bitset *dst, struct cne_acl_bitset *src, bits_t mask)
{
    uint32_t n;

    for (n = 0; n < CNE_ACL_BIT_SET_SIZE; n++)
 ...
 */
static int
acl_exclude(struct cne_acl_bitset *dst, struct cne_acl_bitset *src1, struct cne_acl_bitset *src2)
{
    uint32_t n;
    bits_t all_bits = 0;
 ...
 */
static int
acl_add_ptr(struct acl_build_context *context, struct cne_acl_node *node, struct cne_acl_node *ptr,
            struct cne_acl_bitset *bits)
{
    uint32_t n, num_ptrs;
    struct cne_acl_ptr_set *ptrs = NULL;
 ...
 */
static int
acl_add_ptr_range(struct acl_build_context *context, struct cne_acl_node *root,
                  struct cne_acl_node *node, uint8_t low, uint8_t high)
{
    uint32_t n;
    struct cne_acl_bitset bitset;
    ...
    for (n = 0; n < UINT8_MAX + 1; n++)
        if (n >= low && n <= high)
            bitset.bits[n / (sizeof(bits_t) * 8)] |= 1U << (n % (sizeof(bits_t) * CHAR_BIT));
 ...
 */
static int
acl_gen_mask(struct cne_acl_bitset *bitset, uint32_t value, uint32_t mask)
{
    int range = 0;
    uint32_t n;
 ...
 */
static int
acl_intersect_type(const struct cne_acl_bitset *a_bits, const struct cne_acl_bitset *b_bits,
                   struct cne_acl_bitset *intersect)
{
    uint32_t n;
    ...
    n = (intersect_bits == 0 ? ACL_INTERSECT_NONE : ACL_INTERSECT) |
        (b_superset == 0 ? 0 : ACL_INTERSECT_B) | (a_superset == 0 ? 0 : ACL_INTERSECT_A);
 ...
 */
static struct cne_acl_node *
acl_dup_node(struct acl_build_context *context, struct cne_acl_node *node)
{
    uint32_t n;
    ...
    if (node->num_ptrs > 0) {
        next->ptrs     = acl_build_alloc(context, node->max_ptrs, sizeof(struct cne_acl_ptr_set));
 ...
 */
static void
acl_deref_ptr(struct acl_build_context *context, struct cne_acl_node *node, int index)
{
    struct cne_acl_node *ref_node;
...
/*
 * acl_exclude cne_acl_bitset from src and copy remaining pointer to dst
 */
static int
acl_copy_ptr(struct acl_build_context *context, struct cne_acl_node *dst, struct cne_acl_node *src,
             int index, struct cne_acl_bitset *b_bits)
{
    int rc;
    struct cne_acl_bitset bits;
 ...
 */
static void
acl_compact_node_ptrs(struct cne_acl_node *node_a)
{
    uint32_t n;
    ...
    for (n = min_add; n + 1 < node_a->num_ptrs; n++) {
        ...
        if (node_a->ptrs[n].ptr == NULL) {
            ...
            acl_include(&node_a->ptrs[n].values, &node_a->ptrs[node_a->num_ptrs - 1].values, 0);
...
static int
acl_resolve_leaf(struct acl_build_context *context, struct cne_acl_node *node_a,
                 struct cne_acl_node *node_b, struct cne_acl_node **node_c)
{
    uint32_t n;
 ...
 */
static int
acl_merge_trie(struct acl_build_context *context, struct cne_acl_node *node_a,
               struct cne_acl_node *node_b, uint32_t level, struct cne_acl_node **return_c)
{
    uint32_t n, m, ptrs_c, ptrs_b;
    uint32_t min_add_c, min_add_b;
    int node_intersect_type;
    struct cne_acl_bitset node_intersect;
    struct cne_acl_node *node_c;
    struct cne_acl_node *node_a_next;
    int node_b_refs;
    int node_a_refs;
     ...
     */
    node_intersect_type = acl_intersect_type(&node_c->values, &node_b->values, &node_intersect);
    ...
    if (node_intersect_type & ACL_INTERSECT) {
        ...
        for (n = 0; n < ptrs_c; n++) {
            ...
            for (m = 0; m < ptrs_b; m++) {
                ...
                struct cne_acl_bitset child_intersect;
                int child_intersect_type;
                struct cne_acl_node *child_node_c = NULL;

                if (node_b->ptrs[m].ptr == NULL || node_c->ptrs[n].ptr == node_b->ptrs[m].ptr)
                    continue;
                ...
                if ((child_intersect_type & ACL_INTERSECT) != 0) {
                    if (acl_merge_trie(context, node_c->ptrs[n].ptr, node_b->ptrs[m].ptr, level + 1,
                                       &child_node_c))
                        return 1;
 ...
 */
static void
acl_build_reset(struct cne_acl_ctx *ctx)
{
    void *rt_fields_start;
    size_t rt_fields_size;

    free(ctx->mem);

    /* A more succinct way of writing the following is:
     *
     *     memset(&ctx->num_categories, 0, rt_fields_size);
     *
     * However, at least one static code analyzer is convinced the intent is
     * only to memset the four bytes pointed to by num_categories. We actually
     * want to clear all bytes in ctx starting at num_categories. The following
     * "ctx + offsetof(..., num_categories)" does what we want and avoids the
     * false positive from the static code analyzer.
     */
    rt_fields_start = (void *)((uintptr_t)ctx + offsetof(struct cne_acl_ctx, num_categories));
    rt_fields_size  = sizeof(*ctx) - offsetof(struct cne_acl_ctx, num_categories);
    memset(rt_fields_start, 0, rt_fields_size);
}

static void
acl_gen_full_range(struct acl_build_context *context, struct cne_acl_node *root,
                   struct cne_acl_node *end, int size, int level)
{
    struct cne_acl_node *node, *prev;
    uint32_t n;
...
static void
acl_gen_range_mdl(struct acl_build_context *context, struct cne_acl_node *root,
                  struct cne_acl_node *end, uint8_t lo, uint8_t hi, int size, int level)
{
    struct cne_acl_node *node;
...
static void
acl_gen_range_low(struct acl_build_context *context, struct cne_acl_node *root,
                  struct cne_acl_node *end, const uint8_t *lo, int size, int level)
{
    struct cne_acl_node *node;
    uint32_t n;
...
static void
acl_gen_range_high(struct acl_build_context *context, struct cne_acl_node *root,
                   struct cne_acl_node *end, const uint8_t *hi, int size, int level)
{
    struct cne_acl_node *node;
    uint32_t n;
...
static struct cne_acl_node *
acl_gen_range_trie(struct acl_build_context *context, const void *min, const void *max, int size,
                   int level, struct cne_acl_node **pend)
{
    int32_t k, n;
    uint8_t hi_ff, lo_00;
    struct cne_acl_node *node, *prev, *root;
    const uint8_t *lo;
    const uint8_t *hi;
...
static struct cne_acl_node *
acl_gen_mask_trie(struct acl_build_context *context, const void *value, const void *mask, int size,
                  int level, struct cne_acl_node **pend)
{
    int32_t n;
    struct cne_acl_node *root;
    struct cne_acl_node *node, *prev;
    struct cne_acl_bitset bits;
...
static struct cne_acl_node *
build_trie(struct acl_build_context *context, struct cne_acl_build_rule *head,
           struct cne_acl_build_rule **last, uint32_t *count)
{
    uint32_t n, m;
    int field_index, node_count;
    struct cne_acl_node *trie;
    struct cne_acl_build_rule *prev, *rule;
    struct cne_acl_node *end, *merge, *root, *end_prev;
    const struct cne_acl_field *fld;
    ...
    *last = prev;
    ...
    while (rule != NULL) {
        ...
        for (n = 0; n < rule->config->num_fields; n++) {
            ...
            case CNE_ACL_FIELD_TYPE_BITMASK:
                merge = acl_gen_mask_trie(context, &fld->value, &fld->mask_range,
                                          rule->config->defs[n].size, end->level + 1, &end);
                break;

            case CNE_ACL_FIELD_TYPE_MASK: {
                /*
                 * set msb for the size of the field and
                 * all higher bits.
                 */
                uint64_t mask;
                mask = CNE_ACL_MASKLEN_TO_BITMASK(fld->mask_range.u32, rule->config->defs[n].size);

                /* gen a mini-trie for this field */
                merge = acl_gen_mask_trie(context, &fld->value, (char *)&mask,
                                          rule->config->defs[n].size, end->level + 1, &end);
            } break;

            case CNE_ACL_FIELD_TYPE_RANGE:
            ...
            default:
                acl_free_node(context, root);
                acl_free_node(context, trie);
                CNE_NULL_RET("Error in rule[%u] type - %hhu\n", rule->f->data.userdata,
...
static void
acl_calc_wildness(struct cne_acl_build_rule *head, const struct cne_acl_config *config)
{
    uint32_t n;
    struct cne_acl_build_rule *rule;
    ...
    for (rule = head; rule != NULL; rule = rule->next) {
        ...
        for (n = 0; n < config->num_fields; n++) {
            ...
            uint64_t msk_val                = CNE_LEN2MASK(bit_len, typeof(msk_val));
            double size                     = bit_len;
            int field_index                 = config->defs[n].field_index;
            const struct cne_acl_field *fld = rule->f->field + field_index;

            switch (rule->config->defs[n].type) {
            case CNE_ACL_FIELD_TYPE_BITMASK:
                wild = (size - __builtin_popcountll(fld->mask_range.u64 & msk_val)) / size;
                break;
...
static void
acl_rule_stats(struct cne_acl_build_rule *head, struct cne_acl_config *config)
{
    struct cne_acl_build_rule *rule;
    uint32_t n, m, fields_deactivated = 0;
    uint32_t start = 0, deactivate = 0;
    int tally[CNE_ACL_MAX_LEVELS][TALLY_NUM];
    ...
    for (rule = head; rule != NULL; rule = rule->next) {
        ...
        for (n = 0; n < config->num_fields; n++) {
            ...
            tally[n][TALLY_0]++;
            for (m = 1; m < CNE_DIM(wild_limits); m++) {
    ...
    if (fields_deactivated) {
        ...
        for (k = 0; k < config->num_fields; k++) {
            if (tally[k][TALLY_DEACTIVATED] == 0) {
                ...
                memmove(&config->defs[l++], &config->defs[k], sizeof(struct cne_acl_field_def));
            }
        }
        config->num_fields = l;
    }
...
static int
rule_cmp_wildness(struct cne_acl_build_rule *r1, struct cne_acl_build_rule *r2)
{
    uint32_t n;
...
/*
 * Split the cne_acl_build_rule list into two lists.
 */
static void
rule_list_split(struct cne_acl_build_rule *source, struct cne_acl_build_rule **list_a,
                struct cne_acl_build_rule **list_b)
{
    struct cne_acl_build_rule *fast;
    struct cne_acl_build_rule *slow;
 ...
 */
static struct cne_acl_build_rule *
rule_list_sorted_merge(struct cne_acl_build_rule *a, struct cne_acl_build_rule *b)
{
    struct cne_acl_build_rule *result     = NULL;
    struct cne_acl_build_rule **last_next = &result;
 ...
 */
static struct cne_acl_build_rule *
sort_rules(struct cne_acl_build_rule *head)
{
    struct cne_acl_build_rule *a;
    struct cne_acl_build_rule *b;
...
static uint32_t
acl_build_index(const struct cne_acl_config *config, uint32_t *data_index)
{
    uint32_t n, m;
    int32_t last_header;
...
build_one_trie(struct acl_build_context *context,
               struct cne_acl_build_rule *rule_sets[CNE_ACL_MAX_TRIES], uint32_t n,
               int32_t node_max)
{
    struct cne_acl_build_rule *last;
    struct cne_acl_config *config;
    ...
    context->tries[n].type  = CNE_ACL_FULL_TRIE;
    context->tries[n].count = 0;

    context->tries[n].num_data_indexes = acl_build_index(config, context->data_indexes[n]);
...
static int
acl_build_tries(struct acl_build_context *context, struct cne_acl_build_rule *head)
{
    uint32_t n, num_tries;
    struct cne_acl_config *config;
    struct cne_acl_build_rule *last;
    struct cne_acl_build_rule *rule_sets[CNE_ACL_MAX_TRIES];

    config       = head->config;
    rule_sets[0] = head;

    /* initialize tries */
    for (n = 0; n < CNE_DIM(context->tries); n++) {
        context->tries[n].type     = CNE_ACL_UNUSED_TRIE;
        context->bld_tries[n].trie = NULL;
        context->tries[n].count    = 0;
    }

    context->tries[0].type = CNE_ACL_FULL_TRIE;
    ...
    for (n = 0;; n = num_tries) {
        ...
        if (context->bld_tries[n].trie == NULL)
            CNE_ERR_RET_VAL(-ENOMEM, "Build of %u-th trie failed\n", n);

        /* Build of the last trie completed. */
        if (last == NULL)
            break;

        if (num_tries == CNE_DIM(context->tries))
            CNE_ERR_RET_VAL(-ENOMEM, "Exceeded max number of tries: %u\n", num_tries);
...
acl_build_log(const struct acl_build_context *ctx)
{
    ...
    for (n = 0; n < CNE_DIM(ctx->tries); n++) {
        if (ctx->tries[n].count != 0)
            CNE_LOG(DEBUG, "trie %u: number of rules: %u, indexes: %u\n", n, ctx->tries[n].count,
...
{
    struct cne_acl_build_rule *br, *head;
    const struct cne_acl_rule *rule;
    uint32_t *wp;
    uint32_t fn, i, n, num;
    size_t ofs, sz;
    ...
    for (i = 0; i != n; i++) {
        rule = (const struct cne_acl_rule *)((uintptr_t)bcx->acx->rules + bcx->acx->rule_sz * i);
 ...
 */
static void
acl_set_data_indexes(struct cne_acl_ctx *ctx)
{
    uint32_t i, n, ofs;
    ...
    for (i = 0; i != ctx->num_tries; i++) {
        ...
        memcpy(ctx->data_indexes + ofs, ctx->trie[i].data_index, n * sizeof(ctx->data_indexes[0]));
        ctx->trie[i].data_index = ctx->data_indexes + ofs;
        ofs += CNE_ACL_MAX_FIELDS;
    }
}
 ...
 */
static int
acl_bld(struct acl_build_context *bcx, struct cne_acl_ctx *ctx, const struct cne_acl_config *cfg,
        uint32_t node_max)
{
    int32_t rc;
    ...
    bcx->cfg            = *cfg;
    bcx->category_mask  = CNE_LEN2MASK(bcx->cfg.num_categories, typeof(bcx->category_mask));
    ...
    if (rc != 0)
        CNE_ERR_RET_VAL(rc, "ACL context: %s, %s() failed with error code: %d\n", bcx->acx->name,
                        __func__, rc);
 ...
 */
static int
acl_check_bld_param(struct cne_acl_ctx *ctx, const struct cne_acl_config *cfg)
{
    ...
    if (ctx == NULL || cfg == NULL || cfg->num_categories == 0 ||
        cfg->num_categories > CNE_ACL_MAX_CATEGORIES || cfg->num_fields == 0 ||
        cfg->num_fields > CNE_ACL_MAX_FIELDS)
        return -EINVAL;

    for (i = 0; i != cfg->num_fields; i++) {
        if (cfg->defs[i].type > CNE_ACL_FIELD_TYPE_BITMASK)
            CNE_ERR_RET_VAL(-EINVAL, "ACL context: %s, invalid type: %hhu for %u-th field\n",
                            ctx->name, cfg->defs[i].type, i);
        for (j = 0; j != CNE_DIM(field_sizes) && cfg->defs[i].size != field_sizes[j]; j++)
            ;

        if (j == CNE_DIM(field_sizes))
            CNE_ERR_RET_VAL(-EINVAL, "ACL context: %s, invalid size: %hhu for %u-th field\n",
...
int
cne_acl_build(struct cne_acl_ctx *ctx, const struct cne_acl_config *cfg)
{
    int32_t rc;
    uint32_t n;
    size_t max_size;
    ...
    for (rc = -ERANGE; n >= NODE_MIN && rc == -ERANGE; n /= 2) {
        ...
        if (rc == 0) {
            ...
            rc = cne_acl_gen(ctx, bcx.tries, bcx.bld_tries, bcx.num_tries, bcx.cfg.num_categories,
                             CNE_ACL_MAX_FIELDS * CNE_DIM(bcx.tries) * sizeof(ctx->data_indexes[0]),
                             max_size);
            if (rc == 0) {
                /* set data indexes. */
