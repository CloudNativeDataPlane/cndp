...
#include <cne_log.h>              // for CNE_LOG_ERR, CNE_ERR_GOTO, CNE_LOG_WARNING
...
static TAILQ_HEAD(cne_hmap_list, hmap) hmap_list;
...
hmap_create(const char *name, uint32_t max_capacity, hmap_funcs_t *funcs)
{
    ...
    if (strnlen(name, HMAP_MAX_NAME_SIZE) >= HMAP_MAX_NAME_SIZE)
        CNE_NULL_RET("Name is greater than or equal to %d bytes\n", HMAP_MAX_NAME_SIZE);
...
_print_kvp(FILE *f, hmap_kvp_t *kvp)
{
    ...
    cne_fprintf(f, "[yellow]Key[]: [green]%-42s[]  [yellow]value[]: [magenta]", buff);
...
hmap_dump(FILE *f, hmap_t *hmap, int sort)
{
    ...
    cne_fprintf(f, "\n[yellow]****[] Hashmap dump ([green]%s[]) [red]%s[] [yellow]****[]\n",
    ...
    cne_fprintf(f, "[green]Total count[]: [red]%u[]\n", hmap_count(hmap));
}

void
hmap_list_dump(FILE *f, int sort)
{
    hmap_t *hmap, *tvar;
