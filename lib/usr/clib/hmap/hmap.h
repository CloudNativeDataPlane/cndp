 ...
 *   0 on success or -1 on error.
 */
CNDP_API int hmap_add(hmap_t *hmap, hmap_type_t type, const char *prefix, const char *key,
                      hmap_val_t *val);
...
__get_kvp(hmap_t *hmap, const char *prefix, const char *key, hmap_type_t type)
{
    ...
    if (!hmap)
        CNE_NULL_RET("get failed - hmap not defined %s(%s)\n", prefix ? prefix : "", key);

    kvp = hmap_kvp_lookup(hmap, prefix, key);
    if (!kvp || kvp->type != type)
        CNE_NULL_RET("get failed for %s(%s)\n", prefix ? prefix : "", key);
 ...
 */
static inline int
hmap_get_pointer(hmap_t *hmap, const char *prefix, const char *key, void **val)
{
