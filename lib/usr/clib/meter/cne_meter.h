 ...
 */
enum cne_color {
    CNE_COLOR_GREEN = 0, /**< Green */
    CNE_COLOR_YELLOW,    /**< Yellow */
    CNE_COLOR_RED,       /**< Red */
 ...
 */
CNDP_API int cne_meter_srtcm_profile_config(struct cne_meter_srtcm_profile *p,
 ...
 *    0 upon success, error code otherwise
 */
CNDP_API int cne_meter_trtcm_profile_config(struct cne_meter_trtcm_profile *p,
 ...
 *    0 upon success, error code otherwise
 */
CNDP_API int cne_meter_trtcm_rfc4115_profile_config(struct cne_meter_trtcm_rfc4115_profile *p,
 ...
 *    0 upon success, error code otherwise
 */
CNDP_API int cne_meter_srtcm_config(struct cne_meter_srtcm *m, struct cne_meter_srtcm_profile *p);
 ...
 *    0 upon success, error code otherwise
 */
CNDP_API int cne_meter_trtcm_config(struct cne_meter_trtcm *m, struct cne_meter_trtcm_profile *p);
 ...
 *    0 upon success, error code otherwise
 */
CNDP_API int cne_meter_trtcm_rfc4115_config(struct cne_meter_trtcm_rfc4115 *m,
 ...
 */
static inline enum cne_color cne_meter_srtcm_color_blind_check(struct cne_meter_srtcm *m,
                                                               struct cne_meter_srtcm_profile *p,
 ...
 *    Color assigned to the current IP packet
 */
static inline enum cne_color cne_meter_srtcm_color_aware_check(struct cne_meter_srtcm *m,
                                                               struct cne_meter_srtcm_profile *p,
                                                               uint64_t time, uint32_t pkt_len,
                                                               enum cne_color pkt_color);
 ...
 *    Color assigned to the current IP packet
 */
static inline enum cne_color cne_meter_trtcm_color_blind_check(struct cne_meter_trtcm *m,
                                                               struct cne_meter_trtcm_profile *p,
                                                               uint64_t time, uint32_t pkt_len);
 ...
 *    Color assigned to the current IP packet
 */
static inline enum cne_color cne_meter_trtcm_color_aware_check(struct cne_meter_trtcm *m,
                                                               struct cne_meter_trtcm_profile *p,
                                                               uint64_t time, uint32_t pkt_len,
                                                               enum cne_color pkt_color);
 ...
 *    Color assigned to the current IP packet
 */
static inline enum cne_color
cne_meter_trtcm_rfc4115_color_blind_check(struct cne_meter_trtcm_rfc4115 *m,
                                          struct cne_meter_trtcm_rfc4115_profile *p, uint64_t time,
                                          uint32_t pkt_len);
 ...
 */
static inline enum cne_color
cne_meter_trtcm_rfc4115_color_aware_check(struct cne_meter_trtcm_rfc4115 *m,
                                          struct cne_meter_trtcm_rfc4115_profile *p, uint64_t time,
                                          uint32_t pkt_len, enum cne_color pkt_color);
...
static inline enum cne_color
cne_meter_srtcm_color_blind_check(struct cne_meter_srtcm *m, struct cne_meter_srtcm_profile *p,
                                  uint64_t time, uint32_t pkt_len)
{
    ...
    if (tc >= pkt_len) {
        m->tc = tc - pkt_len;
        m->te = te;
        return CNE_COLOR_GREEN;
    }

    if (te >= pkt_len) {
        m->tc = tc;
        m->te = te - pkt_len;
        return CNE_COLOR_YELLOW;
    }

    m->tc = tc;
    m->te = te;
    return CNE_COLOR_RED;
}

static inline enum cne_color
cne_meter_srtcm_color_aware_check(struct cne_meter_srtcm *m, struct cne_meter_srtcm_profile *p,
                                  uint64_t time, uint32_t pkt_len, enum cne_color pkt_color)
{
    uint64_t time_diff, n_periods, tc, te;
    ...
    /* Color logic */
    if ((pkt_color == CNE_COLOR_GREEN) && (tc >= pkt_len)) {
        m->tc = tc - pkt_len;
        m->te = te;
        return CNE_COLOR_GREEN;
    }

    if ((pkt_color != CNE_COLOR_RED) && (te >= pkt_len)) {
        m->tc = tc;
        m->te = te - pkt_len;
        return CNE_COLOR_YELLOW;
    }
...
static inline enum cne_color
cne_meter_trtcm_color_blind_check(struct cne_meter_trtcm *m, struct cne_meter_trtcm_profile *p,
                                  uint64_t time, uint32_t pkt_len)
{
...
static inline enum cne_color
cne_meter_trtcm_color_aware_check(struct cne_meter_trtcm *m, struct cne_meter_trtcm_profile *p,
                                  uint64_t time, uint32_t pkt_len, enum cne_color pkt_color)
{
    uint64_t time_diff_tc, time_diff_tp, n_periods_tc, n_periods_tp, tc, tp;
    ...
    /* Color logic */
    if ((pkt_color == CNE_COLOR_RED) || (tp < pkt_len)) {
        m->tc = tc;
        m->tp = tp;
        return CNE_COLOR_RED;
    }

    if ((pkt_color == CNE_COLOR_YELLOW) || (tc < pkt_len)) {
        m->tc = tc;
        m->tp = tp - pkt_len;
        return CNE_COLOR_YELLOW;
    }
