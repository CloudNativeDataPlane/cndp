...
#ifndef _CNE_GRAPH_PRIVATE_H_
#define _CNE_GRAPH_PRIVATE_H_
...
#define GRAPH_LOG(level, ...)                    \
    cne_log(CNE_LOG_##level, __func__, __LINE__, \
            CNE_FMT("GRAPH: " CNE_FMT_HEAD(__VA_ARGS__, ) "\n", CNE_FMT_TAIL(__VA_ARGS__, )))
...
struct node {
    ...
    cne_node_t id;                        /**< Allocated identifier for the node. */
    cne_node_t parent_id;                 /**< Parent node identifier. */
    cne_edge_t nb_edges;                  /**< Number of edges from this node. */
    char next_nodes[][CNE_NODE_NAMESIZE]; /**< Names of next nodes. */
};
...
struct graph {
    ...
    cne_graph_off_t nodes_start;   /**< Node memory start offset in graph reel. */
    cne_node_t src_node_count;     /**< Number of source nodes in a graph. */
    struct cne_graph *graph;       /**< Pointer to graph data. */
    cne_node_t node_count;         /**< Total number of nodes. */
    uint32_t cir_start;            /**< Circular buffer start offset in graph reel. */
    uint32_t cir_mask;             /**< Circular buffer mask for wrap around. */
    cne_graph_t id;                /**< Graph identifier. */
    size_t mem_sz;                 /**< Memory size of the graph. */
 ...
 * @internal
 *
 * Checks whether node in the graph has a edge to itself i.e. forms a loop.
 *
 * @param graph
 *   Pointer to the internal graph object.
 *
 ...
 */
cne_node_t graph_src_nodes_count(struct graph *graph);
 ...
 *   Number of nodes.
 */
cne_node_t graph_nodes_count(struct graph *graph);
 ...
 *
 * @param graph
 *   Pointer to cne_graph object.
 * @param id
 *   Node Identifier.
 *
 * @return
 *   Pointer to cne_node if identifier is valid else NULL.
 */
struct cne_node *graph_node_id_to_ptr(const struct cne_graph *graph, cne_node_t id);
 ...
 *   Pointer to cne_node if identifier is valid else NULL.
 */
struct cne_node *graph_node_name_to_ptr(const struct cne_graph *graph, const char *node_name);
 ...
 * @param hdr
 *   Flag if true to print the header information
 */
void node_dump(FILE *f, struct node *n, bool hdr);

#endif /* _CNE_GRAPH_PRIVATE_H_ */
