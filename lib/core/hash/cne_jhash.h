...
#define __cne_jhash_mix(a, b, c) do { \
    a -= c; a ^= rot(c, 4); c += b; \
    b -= a; b ^= rot(a, 6); a += c; \
    c -= b; c ^= rot(b, 8); b += a; \
    a -= c; a ^= rot(c, 16); c += b; \
...
#define __cne_jhash_final(a, b, c) do { \
    c ^= b; c -= rot(b, 14); \
    a ^= c; a -= rot(c, 11); \
    b ^= a; b -= rot(a, 25); \
    c ^= b; c -= rot(b, 16); \
...
#define LOWER8b_MASK  le32toh(0xff)
#define LOWER16b_MASK le32toh(0xffff)
#define LOWER24b_MASK le32toh(0xffffff)

static inline void
__cne_jhash_2hashes(const void *key, uint32_t length, uint32_t *pc, uint32_t *pb,
                    unsigned check_align)
{
    uint32_t a, b, c;

    /* Set up the internal state */
    a = b = c = CNE_JHASH_GOLDEN_RATIO + ((uint32_t)length) + *pc;
    c += *pb;
 ...
 */
static inline void
cne_jhash_2hashes(const void *key, uint32_t length, uint32_t *pc, uint32_t *pb)
{
    __cne_jhash_2hashes(key, length, pc, pb, 1);
}

/**
 * Same as cne_jhash_32b, but takes two seeds and return two uint32_ts.
 * pc and pb must be non-null, and *pc and *pb must both be initialized
 * with seeds. If you pass in (*pb)=0, the output (*pc) will be
 * the same as the return value from cne_jhash_32b.
 *
 * @param k
 *   Key to calculate hash of.
 * @param length
 ...
 */
static inline void
cne_jhash_32b_2hashes(const uint32_t *k, uint32_t length, uint32_t *pc, uint32_t *pb)
{
    __cne_jhash_2hashes((const void *)k, (length << 2), pc, pb, 0);
}
...
cne_jhash(const void *key, uint32_t length, uint32_t initval)
{
    uint32_t initval2 = 0;

    cne_jhash_2hashes(key, length, &initval, &initval2);
 ...
 */
static inline uint32_t
cne_jhash_32b(const uint32_t *k, uint32_t length, uint32_t initval)
{
    uint32_t initval2 = 0;

    cne_jhash_32b_2hashes(k, length, &initval, &initval2);

    return initval;
}

static inline uint32_t
__cne_jhash_3words(uint32_t a, uint32_t b, uint32_t c, uint32_t initval)
{
    a += CNE_JHASH_GOLDEN_RATIO + initval;
    b += CNE_JHASH_GOLDEN_RATIO + initval;
    c += CNE_JHASH_GOLDEN_RATIO + initval;
 ...
 */
static inline uint32_t
cne_jhash_3words(uint32_t a, uint32_t b, uint32_t c, uint32_t initval)
{
    return __cne_jhash_3words(a + 12, b + 12, c + 12, initval);
}
 ...
 */
static inline uint32_t
cne_jhash_2words(uint32_t a, uint32_t b, uint32_t initval)
{
    return __cne_jhash_3words(a + 8, b + 8, 8, initval);
}
 ...
 */
static inline uint32_t
cne_jhash_1word(uint32_t a, uint32_t initval)
{
    return __cne_jhash_3words(a + 4, 4, 4, initval);
}
