...
#ifndef _CNE_FBK_HASH_H_
#define _CNE_FBK_HASH_H_
...
#include <errno.h>         // for ENOENT, ENOSPC
#include <sys/queue.h>
...
#ifndef CNE_FBK_HASH_INIT_VAL_DEFAULT
...
struct cne_fbk_hash_params {
    ...
    cne_fbk_hash_fn hash_func;   /**< The hash function. */
...
struct cne_fbk_hash_table {
    char name[CNE_FBK_HASH_NAMESIZE]; /**< Name of the hash. */
    uint32_t entries;                 /**< Total number of entries. */
    uint32_t entries_per_bucket;      /**< Number of entries in a bucket. */
...
cne_fbk_hash_get_bucket(const struct cne_fbk_hash_table *ht, uint32_t key)
{
    return (ht->hash_func(key, ht->init_val) & ht->bucket_mask) << ht->bucket_shift;
}
 ...
 */
static inline int
cne_fbk_hash_add_key_with_bucket(struct cne_fbk_hash_table *ht, uint32_t key, uint16_t value,
                                 uint32_t bucket)
{
    /*
    ...
    const uint64_t new_entry =
        ((uint64_t)(key) << 32) | ((uint64_t)(value) << 16) | 1; /* 1 = is_entry bit. */
    uint32_t i;
 ...
 */
static inline int
cne_fbk_hash_add_key(struct cne_fbk_hash_table *ht, uint32_t key, uint16_t value)
{
    return cne_fbk_hash_add_key_with_bucket(ht, key, value, cne_fbk_hash_get_bucket(ht, key));
}
 ...
 */
static inline int
cne_fbk_hash_delete_key_with_bucket(struct cne_fbk_hash_table *ht, uint32_t key, uint32_t bucket)
{
 ...
 */
static inline int
cne_fbk_hash_delete_key(struct cne_fbk_hash_table *ht, uint32_t key)
{
    return cne_fbk_hash_delete_key_with_bucket(ht, key, cne_fbk_hash_get_bucket(ht, key));
}
 ...
 */
static inline int
cne_fbk_hash_lookup_with_bucket(const struct cne_fbk_hash_table *ht, uint32_t key, uint32_t bucket)
{
    union cne_fbk_hash_entry current_entry;
    uint32_t i;
 ...
 */
static inline int
cne_fbk_hash_lookup(const struct cne_fbk_hash_table *ht, uint32_t key)
{
    return cne_fbk_hash_lookup_with_bucket(ht, key, cne_fbk_hash_get_bucket(ht, key));
}
 ...
 */
static inline void
cne_fbk_hash_clear_all(struct cne_fbk_hash_table *ht)
{
...
cne_fbk_hash_get_load_factor(struct cne_fbk_hash_table *ht)
{
 ...
 *
 * @return
 *   pointer to hash table structure or NULL on error with errno
 *   set appropriately. Possible errno values include:
 ...
 *   operations, or NULL on error with errno set appropriately.
 *   Possible errno error values include:
