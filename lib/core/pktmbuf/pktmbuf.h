...
pktmbuf_metadata_bufsz(const pktmbuf_t *m)
{
    ...
    if (((p = m->pooldata) != NULL) && p->metadata)
        return p->metadata_bufsz;
    else
        return (uint32_t)pktmbuf_headroom(m);
}
 ...
 * @param bufsz
 *   Size of each buffer in the pktmbuf pool, must not be zero
 * @param cache_sz
...
{
    if (c) {
        if (addr && bufcnt && bufsz) {
            c->addr     = addr;
            c->bufcnt   = bufcnt;
            c->bufsz    = bufsz;
            c->cache_sz = cache_sz; /* Can be zero */
            c->ops      = ops;      /* Can be NULL */
            if (metadata) {         /* Can be NULL, use internal metadata */
                c->metadata = metadata;

                /* Size of metadata buffer should be a multiple of a cacheline */
                if (metadata_bufsz && (metadata_bufsz % CNE_CACHE_LINE_SIZE) == 0) {
