...
func collectStats(handle *cne.System, lportName string, ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
...
func receivePackets(handle *cne.System, lportName string, ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
	...
	for {
		...
		default:
			...
			if size > 0 {
				cne.FreePacketBuffer(packets[:size])
			}
		}
	}
}
...
func verifyIPv4(l2 unsafe.Pointer) bool {
	...
	ethWords := getWords(l2, cne.EtherLen, 0)
	if !(len(ethWords) < cne.EtherLen/2) {
		if cne.SwapBytesUint16(ethWords[6]) == cne.ETHER_TYPE_IPV4 {
			return true
		}
	}
...
func calculateDataChecksum(data []uint16) uint32 {
	var sum uint32
	for i := range data {
		sum += uint32(cne.SwapBytesUint16(data[i]))
	}
	return sum
}
...
func verifyIPv4Checksum(l3 unsafe.Pointer) bool {
	...
	ipv4Words := getWords(l3, cne.IPv4Len, 0)
	if !(len(ipv4Words) < cne.IPv4Len/2) {
		temp := ipv4Words[5]
		ipv4Words[5] = 0
		checksum := ^reduceChecksum(calculateDataChecksum(ipv4Words))
		ipv4Words[5] = temp
		if checksum == cne.SwapBytesUint16(ipv4Words[5]) {
			return true
		}
	}
	return false
...
func verifyIPv4ChecksumPackets(handle *cne.System, lportName string, ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
...
func transmitPackets(handle *cne.System, lportName string, ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
...
func reTransmitPackets(handle *cne.System, lportName string, ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
...
func startCNDP(configStr string) (handle *cne.System) {
	handle, err := cne.Open(configStr)
	if err != nil {
