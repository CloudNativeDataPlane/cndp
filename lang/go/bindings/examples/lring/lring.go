...
func CreateElem(name string, elementSz, count uint, strFlags []string) (*LocklessRing, error) {
...
//
// Returns the internal lockless ring object or error value is set
func Create(name string, count uint, strFlags []string) (*LocklessRing, error) {

	if count == 0 {
		return nil, fmt.Errorf("Count is zero")
	}
	return CreateElem(name, C.RING_DEFAULT_ELEM_SZ, count, strFlags)
}
